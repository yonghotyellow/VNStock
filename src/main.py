...
import pandas as pd
import argparse
from data_utils import get_companies, get_company_info, get_officers, get_shareholders, get_dividends, get_stock_quote_history
...
def main(is_test):
    ...
    # get_company_info(companies_df, COMPANY_INFO_FILE, is_test)

    # Fetch and save officers data
    # get_officers(companies_df, COMPANY_OFFICERS_FILE, is_test)

    # Fetch and save shareholders data
    # get_shareholders(companies_df, COMPANY_SHAREHOLDERS_FILE, is_test)

    # Fetch and save dividends data
    # get_dividends(companies_df, COMPANY_DIVIDENDS_FILE, is_test)

    get_stock_quote_history(companies_df, COMPANY_STOCK_QUOTE_FILE, start_date="2020-01-01", is_test=is_test)
