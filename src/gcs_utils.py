...
def upload_to_gcs(source_file_path, destination_blob_name, credentials_path=None):
    """
    Upload a local file to a specified GCS bucket and path.

    Parameters:
        source_file_path (str): Local path to the file to upload.
        destination_blob_name (str): Destination path in GCS bucket (e.g., "raw/company/2024-04-08/data.parquet").
...
def upload_bytes_to_gcs(byte_buffer, destination_blob_name, credentials_path=None):
    if not BUCKET_NAME:
        raise ValueError("❌ GCS_BUCKET not set in environment variables!")
    client = get_gcs_client(credentials_path or CREDENTIALS_PATH)
    bucket = client.bucket(BUCKET_NAME)
...
def load_parquet_from_gcs(blob_name, credentials_path=None):
    """
